...
if (url.includes("/v1/im/get_recent_chats")) {
  delete obj.data?.recent_chat_user;
} else if (url.includes("/v3/user/me")) {
  if (obj.data) {
      ...
      if (obj.data.hula_tabs?.all_show_tab_config) {
          obj.data.hula_tabs.all_show_tab_config = obj.data.hula_tabs.all_show_tab_config
              .filter(item => item.tab_id !== "note" && item.tab_id !== "atMe");
      }
  }
  ...
  // 整体配置
  const item = ["app_theme", "loading_img", "splash", "store", "ios_2025_cny_shake_config"];
  if (obj?.data) {
    for (let i of item) {
      delete obj.data[i];
    }
